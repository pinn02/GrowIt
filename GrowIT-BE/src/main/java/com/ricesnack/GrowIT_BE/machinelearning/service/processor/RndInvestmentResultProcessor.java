...
public class RndInvestmentResultProcessor implements PredictionResultProcessor {
    ...
    private static final BigDecimal BASE_COST = new BigDecimal("150");
    ...
    public Object process(Map<Long, Float> probabilities, PredictionRequestDto requestDto) {
        ...
        BigDecimal spentCapital = BASE_COST.add(companyValueComponent).setScale(0, RoundingMode.HALF_UP);

        // 4. 자본 확인
        if (saved.getCapital().compareTo(spentCapital) < 0) {
            throw new IllegalStateException("자본이 부족하여 R&D 투자를 진행할 수 없습니다.");
        }
        ...
        // 6. Saved 엔티티의 범용 업데이트 메서드를 호출하여 상태 업데이트 (턴 증가 없음)
