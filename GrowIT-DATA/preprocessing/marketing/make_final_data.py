...
class TimeAligner:
    ...
    def add_df(self, df, dataset_name, data_type='monthly'):
        ...
        # <<< 최종 수정: 컬럼 이름 대신 위치(0, 1)를 사용 >>>
        date_col, data_col = df.columns[0], df.columns[1]
...
def preprocess_quarterly(fp, val_col, cat_name):
    ...
    return df_long[val_col].resample('MS').interpolate().reset_index()
...
if __name__ == '__main__':
    ...
    df_inv = df_inv.groupby(df_inv.columns[1])['돈'].mean().reset_index()
