...
if __name__ == '__main__':
    try:
        ...
        clean_data = pd.read_csv(data_filepath, index_col='year', parse_dates=True)
        ...
        if CHOSEN_MODE == 'classifier':
            ...
            config = {
                ...
                'features': [col for col in features if col not in ['manufacturing_productivity', 'talent_tier']]
            }
            analyzer = RandomForestAnalyzer.RandomForestAnalyzer(clean_data, config)
            analyzer.prepare_modeling_data('manufacturing_productivity')
